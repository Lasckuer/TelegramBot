...
async def _fetch_fiat_rates(session: aiohttp.ClientSession, rates: dict):
    try:
...
async def _fetch_crypto_rates(session: aiohttp.ClientSession, rates: dict):
...
async def get_live_rates() -> dict:
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            await _fetch_fiat_rates(session, rates)
            await _fetch_crypto_rates(session, rates)
