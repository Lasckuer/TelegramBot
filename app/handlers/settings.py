...
async def show_limits(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"Лимит: {LIMIT}р. Введите новое число:", reply_markup=kb_reply.get_cancel_kb())
...
async def set_limit(message: types.Message, state: FSMContext):
    ...
    if message.text.isdigit():
        ...
        await message.answer(f"✅ Лимит изменен: {LIMIT}р", reply_markup=kb_reply.get_main_menu())
        await state.clear()
    else:
